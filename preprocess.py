...
def main_wo_bpe():
    ...
    parser.add_argument('-test_path', type=str, default="test_100.csv")
    ...
    if opt.tokenizer == "spacy":
        ...
        if opt.tk_type == 'unigram':
            ...
            src_sp.Load(os.path.join(spm_dir, "bpe", "train_em_informal_spm_bpe.model"))
            trg_sp.Load(os.path.join(spm_dir, "bpe", "train_em_formal_spm_bpe.model"))
    ...
    train = TabularDataset(path=os.path.join(opt.data_dir,opt.train_path), format="csv", fields=[("src", SRC), ("trg", TRG)])
    val = TabularDataset(path=os.path.join(opt.data_dir, opt.valid_path), format="csv", fields=[("src", SRC), ("trg", TRG)])
    test = TabularDataset(path=os.path.join(opt.data_dir, opt.test_path), format="csv", fields=[("src", SRC), ("trg", TRG)])
